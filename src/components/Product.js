...
import { BsPlus, BsEyeFill, BsAlarm, BsCart, BsCartPlus, BsCart4, BsCartCheckFill, BsCartPlusFill, BsEyeSlash, BsEyedropper, BsZoomIn } from 'react-icons/bs';
...
const Product = ({ product }) => {
  ...
  return (
  <div>
    <div className='border border-[decoration-black-900] h-[300px] mb-4
     ...
     shadow-md overflow-hidden cursor-pointer hover:translate-y-1 transition duration-800'>
      ...
      <div className='font-semibold flex justify-left items-left h-0 py-2'>$ {price}</div>
      </div>
      <div className='w-full h-60 flex justify-center items-center'>
        <div className='w-[200px] mx-auto flex
        justify-center items-center'>
          <img className='cursor-auto max-h-[160px] group-hover:scale-110 
          transition duration-300 cursor-pointer' src={image} alt='' />
        </div>
      </div>
      <div className='absolute -top-3 right-3 group-hover:top-3
    ...
    <div>
      ...
      <Link to={`/product/${id}`}>
        <h2 className='font-semibold text-sm mb-3 hover:text-gray-600'>{title}</h2>
      </Link>
    </div>
  </div>
  );
